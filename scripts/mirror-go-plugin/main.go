	...
	err := filepath.Walk(
		dir, func(path string, info fs.FileInfo, err error) error {
			...
			contents = bytes.ReplaceAll(
				contents,
				[]byte(`"github.com/boba-keyost/sqlc/internal/codegen/golang/opts"`),
				[]byte(`"github.com/boba-keyost/sqlc-gen-go/internal/opts"`),
			)

			contents = bytes.ReplaceAll(
				contents,
				[]byte(`"github.com/boba-keyost/sqlc/internal/plugin"`),
				[]byte(`"github.com/sqlc-dev/plugin-sdk-go/plugin"`),
			)

			contents = bytes.ReplaceAll(
				contents,
				[]byte(`"github.com/boba-keyost/sqlc/internal/codegen/sdk"`),
				[]byte(`"github.com/sqlc-dev/plugin-sdk-go/sdk"`),
			)

			contents = bytes.ReplaceAll(
				contents,
				[]byte(`"github.com/boba-keyost/sqlc/internal/metadata"`),
				[]byte(`"github.com/sqlc-dev/plugin-sdk-go/metadata"`),
			)

			contents = bytes.ReplaceAll(
				contents,
				[]byte(`"github.com/boba-keyost/sqlc/internal/pattern"`),
				[]byte(`"github.com/sqlc-dev/plugin-sdk-go/pattern"`),
			)

			contents = bytes.ReplaceAll(
				contents,
				[]byte(`"github.com/boba-keyost/sqlc/internal/debug"`),
				[]byte(`"github.com/boba-keyost/sqlc-gen-go/internal/debug"`),
			)

			contents = bytes.ReplaceAll(
				contents,
				[]byte(`"github.com/boba-keyost/sqlc/internal/inflection"`),
				[]byte(`"github.com/boba-keyost/sqlc-gen-go/internal/inflection"`),
			)
